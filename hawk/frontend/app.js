...
function renderAll(){
  ...
  renderNSweep(R); renderWeights(R);
...
function renderHeat(R){
  const keys=[...STATE.activeDetectors].filter(k=>R.detectors[k]);
  const n=R.n,W=1000,rowH=26,H=keys.length*rowH+10,labW=130;
  let svg=svgOpen(W,H,true);
  keys.forEach((k,ri)=>{const yTop=5+ri*rowH;
    svg+=`<text class="axis-t" x="6" y="${yTop+rowH/2+3}" fill="${COLORS[k]}">${DET_NAMES[k]}</text>`;
    const sc=R.detectors[k].score,cw=(W-labW)/n;
    for(let i=0;i<n;i++){const a=sc[i];if(a<0.05)continue;
      svg+=`<rect x="${labW+i*cw}" y="${yTop}" width="${Math.max(1,cw+0.5)}" height="${rowH-4}" fill="${COLORS[k]}" opacity="${(a*0.9).toFixed(2)}"/>`;}});
...
function renderVotes(R){
  ...
  const W=400,H=200,padB=30,padL=10,padT=10,mx=Math.max(...hist),bw=(W-padL*2)/(nDet+1);
  let svg=svgOpen(W,H,true);
  hist.forEach((c,i)=>{const h=mx?(c/mx)*(H-padB-padT):0,op=i===0?0.25:0.4+i/(nDet+1)*0.6;
    svg+=`<rect x="${padL+i*bw+4}" y="${H-padB-h}" width="${bw-8}" height="${h}" fill="var(--acid)" opacity="${op.toFixed(2)}" rx="2"/>`;
    svg+=`<text class="axis-t" x="${padL+i*bw+bw/2}" y="${H-padB+14}" text-anchor="middle">${i}표</text>`;
    if(c>0)svg+=`<text class="axis-t" x="${padL+i*bw+bw/2}" y="${H-padB-h-5}" text-anchor="middle" fill="var(--ink)">${c}</text>`;});
...
function renderVarChart(){
  ...
  const mx=Math.max(...data.map(d=>d.rate),1),W=1000,barH=34,H=data.length*barH+10,labW=130;
  let svg=svgOpen(W,H,true);
  data.forEach((d,i)=>{const yy=8+i*barH,w=(d.rate/mx)*(W-labW-90),active=d.col===STATE.activeVar;
    svg+=`<text class="axis-t" x="6" y="${yy+barH/2}" fill="${active?'var(--acid)':'var(--ink)'}" font-size="11">${d.col}</text>`;
    svg+=`<rect x="${labW}" y="${yy+5}" width="${Math.max(2,w)}" height="${barH-16}" fill="var(--acid)" opacity="${active?.85:.4}" rx="2"/>`;
    svg+=`<text class="axis-t" x="${labW+w+8}" y="${yy+barH/2}" fill="var(--dim)">${d.rate.toFixed(1)}% · ${d.count}건</text>`;});
